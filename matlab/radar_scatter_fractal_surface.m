function [sigma_0] = radar_backscatter_fractal_surface(rho,s_l,H,theta_incidence,theta_emission)
%This function solves numerically for the generalized bistatic radar scattering cross-section
...
% theta_incidence = incidence angle
% theta_emission = emission angle
%OUTPUT VARIABLE:
% sigma_0 = radar backscatter cross-section in dB

integrand               = @(x) exp(-4 * pi.^2 .* s_l.^2 .* x.^(2*H) .* cosd(theta_emission).^2) ...
                            .* x .* besselj(0, 2 * pi * x .* (sind(theta_emission - sind(theta_incidence))));
